...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ user: { _id: result.insertedId, username, email, savedBooks: [], joinedCommunities: [] }, token });
  } catch (err) {
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const baseUrl = process.env.APP_URL || process.env.PUBLIC_URL || req.headers.origin || `http://${req.get('host')}`;
