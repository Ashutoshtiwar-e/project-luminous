...
export const fetchAndSeedGoogleBooks = async (query: string, db: any) => {
  try {
    ...
    for (const item of data.items) {
      ...
      const existing = await db.collection('books').findOne({ id });
      if (!existing) {
        const book = {
          id,
          ...
          reviewCount: vol.ratingsCount || 0,
          summary: vol.description || '',
          description: vol.description || '',
          coverImage: vol.imageLinks?.thumbnail || vol.imageLinks?.smallThumbnail || '',
...
export const searchBooks = async (req: Request, res: Response) => {
  try {
    ...
    let results = await db.collection('books').find(filter).skip(skip).limit(limit).toArray();
    if (query && results.length < 5 && page === 1) {
      await fetchAndSeedGoogleBooks(query, db);
      results = await db.collection('books').find(filter).skip(skip).limit(limit).toArray();
    }
    res.json(results);
  } catch (error) {
...
export const getGroupedByCategories = async (req: Request, res: Response) => {
  try {
    ...
    const grouped = await db.collection('books').aggregate([
      ...
      { $project: { _id: 0, category: "$_id", books: "$books" } },
      { $sort: { category: 1 } }
    ]).toArray();
    res.json(grouped);
  } catch (err) {
  ...
  try {
    ...
    if (req.user) {
      ...
      if (user && user.savedBooks && user.savedBooks.length > 0) {
        ...
        const savedGenres = [...new Set(savedBooksData.flatMap((b: any) => b.genre))];
    ...
    console.error(err); res.status(500).json({ error: err.message || err.toString() });
  }
};
