  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-[#f5c518]/30 font-sans flex flex-col">
      ...
      <main className="flex-1 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 w-full">
        ...
        <div className="sticky top-16 z-40 bg-[#0a0a0a]/90 backdrop-blur-md py-4 mb-12 border-b border-white/10 flex flex-wrap justify-center gap-1 sm:gap-2">
              ...
              <div className="flex flex-col gap-y-4">
                {group.books.map((book) => (
                  <Link 
                    key={book.id} 
                    to={`/book/${book.id}`}
                    className="group flex flex-col sm:flex-row gap-6 p-6 rounded-sm bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 hover:border-white/10 transition-all"
                  >
                    <div className="w-24 h-36 sm:w-32 sm:h-48 shrink-0 bg-[#1a1a1a] rounded-sm overflow-hidden">
                      <CoverImage 
                        src={book.coverImage} 
                    ...
                    <div className="flex flex-col justify-center overflow-hidden py-2 w-full">
                      <h3 className="text-xl md:text-2xl font-black uppercase tracking-tighter text-white group-hover:text-[#f5c518] transition-colors mb-2">
                        {cleanAndFormatText(book.title)}
                      </h3>
                      <p className="text-sm text-white/70 line-clamp-3 md:line-clamp-4 leading-relaxed mb-4">
                        {book.summary ? cleanAndFormatText(book.summary) : "No description available."}
                      </p>
                      <div className="mt-auto flex items-center justify-between flex-wrap gap-2">
                        <p className="text-xs font-bold text-white/50 uppercase tracking-widest">
                          {cleanAndFormatText(book.author)} {book.publicationDate ? `• ${new Date(book.publicationDate).getFullYear()}` : ''}
                        </p>
